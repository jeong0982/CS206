...
Each node appears on the correct row list and the correct column list."""
    ...
    for row in range(s.nrows):
      ...
      while p is not None:
        ...
        if p.right is not None:
          self.assertTrue(p.col < p.right.col, consmsg)
        p = p.right
    for col in range(s.ncols):
      p = s._pcol[col]
      while p is not None:
        self.assertEqual(p.col, col, consmsg)
        self.assertTrue((p.row, p.col) in nodes, consmsg)
        del nodes[p.row, p.col] # remove nodes found on column lists
        if p.down is not None:
          self.assertTrue(p.row < p.down.row, consmsg)
        p = p.down
  ...
  def test_transpose_time(self):
    ...
    self.consistency(t)
    self.assertTrue(t1 - t0 < 1.0,
                    "Transpose does not seem to run in linear time")
  ...
  def test_add_time(self):
    ...
    self.consistency(t)
    self.assertTrue(t1 - t0 < 1.0,
                    "Addition does not seem to run in linear time")
